def process_data(excel,area):
  ...
  df_ptr[['date','unit','seam','series','lp_n','lp_e','lp_z','rom','access','v_dist','hz_dist','truck_count']].to_csv('coal_ptr_'+area+'_'+last_update+'.csv', index=True, index_label='id')
  csvt_txt = "\"Integer\",\"Date\",\"String\",\"String\",\"String\",\"CoordY\",\"CoordX\",\"Real\",\"String\",\"String\",\"Real\",\"Real\",\"Real\""
  csvt_file = open('coal_ptr_'+area+'_'+last_update+'.csvt', "w")
  n = csvt_file.write(csvt_txt)
  csvt_file.close()
  
  print('Selesai..check files untuk download .csv dan .csvt')
